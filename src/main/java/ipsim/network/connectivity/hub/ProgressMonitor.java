 ...
 */
public final class ProgressMonitor implements Accessible
{
	 ...
	 * @see JOptionPane
	 */
	public ProgressMonitor(final Component parentComponent,
	...
	private ProgressMonitor(final Component parentComponent,
	                        final Object message,
	                        final String note,
	                        final int min,
	                        final int max,
	                        final ProgressMonitor group)
	{
		this.min=min;
		this.max=max;
	...
	public boolean isCanceled()
	{
		...
		final Object w=pane.getValue();
		return w!=null && cancelOption.length==1 && w.equals(cancelOption[0]);
	}
	...
	/*
		 ...
		 * @since 1.5
		 */
	public AccessibleContext accessibleContext=null;
	 ...
	 * @since 1.5
	 */
	public class AccessibleProgressMonitor extends AccessibleContext
			implements AccessibleText, ChangeListener, PropertyChangeListener
	{
        ...
        public void stateChanged(final ChangeEvent e)
		{
			...
			if (myBar!=null)
			{
				...
				final Object newModelValue=myBar.getValue();
		...
		{
			if (e.getSource().equals(noteLabel) && e.getPropertyName().equals("text"))
			{
		...
		public int getAccessibleChildrenCount()
		{
			...
			final AccessibleContext ac=getPanelAccessibleContext();
			if (ac!=null)
			{
		...
		private AccessibleContext getPanelAccessibleContext()
		{
			if (myBar!=null)
			{
				final Component c=myBar.getParent();
		...
		{
			final AccessibleText at=getNoteLabelAccessibleText();
			if (at!=null && sameWindowAncestor(pane, noteLabel))
			{
				// convert point from the option pane bounds
				// to the note label bounds.
				final Point noteLabelPoint=SwingUtilities.convertPoint(pane,
						p,
						noteLabel);
        ...
        public Rectangle getCharacterBounds(final int i)
		{
			...
			if (at!=null && sameWindowAncestor(pane, noteLabel))
			{
				...
				final Rectangle noteLabelRect=at.getCharacterBounds(i);
		...
		/*
			 ...
			 */
		private boolean sameWindowAncestor(final Component src, final Component dest)
		{
			if (src==null || dest==null)
			{
				return false;
			}
			return SwingUtilities.getWindowAncestor(src).equals(SwingUtilities.getWindowAncestor(dest));
		}
	...
	{
		Frame sharedFrame=
				(Frame)AppContext.getAppContext().get(JOptionPane.class);
		if (sharedFrame==null)
		{
